...
- **Result rendering**: successful execute-command results default to sectioned plain text (real newlines: stdout → `[stderr]` → `[exit code] N` → spill notice), directly readable by humans and models; set `resultFormat: "json"` per connection for structured CommandResult JSON; error paths always stay JSON (`code`/`message`/`retriable`/`partial`/`replaySafe`)
...
- **Dedicated SFTP connection**: `sftpDedicatedConn` (per connection or `$global` default) runs file transfers over their own SSH connection — gateways with a shadow container / session-scoped overlay in front of sshd discard files written over the reused shell session; the connection is created lazily (dialed on first transfer), recycled after 5 idle minutes, and keepalive-probed before reuse (auto-redials if the gateway dropped it)
